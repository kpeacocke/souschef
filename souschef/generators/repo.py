...
context = 3
"""
    safe_write_text(repo_path / "ansible.cfg", repo_path, cfg_content)
...
roles: []
"""
    safe_write_text(repo_path / "requirements.yml", repo_path, requirements_content)
...
*.log
"""
    safe_write_text(repo_path / ".gitignore", repo_path, gitignore_content)
...
*.gz binary
"""
    safe_write_text(repo_path / ".gitattributes", repo_path, gitattributes_content)
...
indent_style = tab
"""
    safe_write_text(repo_path / ".editorconfig", repo_path, editorconfig_content)
...
"""
    safe_write_text(repo_path / "README.md", repo_path, readme_content)
...
def _create_inventory_first_structure(repo_path: Path) -> None:
    ...
    safe_mkdir(
        repo_path / "inventories" / "prod" / "group_vars", repo_path, parents=True
    )
    safe_mkdir(
        repo_path / "inventories" / "prod" / "host_vars", repo_path, parents=True
    )
    safe_mkdir(
        repo_path / "inventories" / "nonprod" / "group_vars", repo_path, parents=True
    )
    safe_mkdir(
        repo_path / "inventories" / "nonprod" / "host_vars", repo_path, parents=True
    )
    safe_mkdir(repo_path / "playbooks", repo_path)
    safe_mkdir(repo_path / "roles", repo_path)
    safe_mkdir(repo_path / "filter_plugins", repo_path)
    safe_mkdir(repo_path / "library", repo_path)
...
"""
    safe_write_text(
        repo_path / "inventories" / "prod" / HOSTS_FILE, repo_path, prod_hosts
    )
    safe_write_text(
        repo_path / "inventories" / "nonprod" / HOSTS_FILE, repo_path, prod_hosts
    )
...
"""
    safe_write_text(
        repo_path / "inventories" / "prod" / "group_vars" / "all.yml",
        repo_path,
        all_vars,
    )
...
"""
    safe_write_text(repo_path / "playbooks" / "site.yml", repo_path, site_playbook)


def _create_playbooks_roles_structure(repo_path: Path) -> None:
    """Create simple playbooks + roles structure."""
    repo_path = _ensure_within_base_path(repo_path, _get_workspace_root())
    safe_mkdir(repo_path / "inventory" / "group_vars", repo_path, parents=True)
    safe_mkdir(repo_path / "inventory" / "host_vars", repo_path, parents=True)
    safe_mkdir(repo_path / "playbooks", repo_path)
    safe_mkdir(repo_path / "roles", repo_path)
...
"""
    safe_write_text(repo_path / "inventory" / "hosts.yml", repo_path, hosts)
...
- name: Deploy application
  ...
  roles:
    - common
"""
    safe_write_text(repo_path / "playbooks" / "deploy.yml", repo_path, deploy_playbook)
...
def _create_collection_structure(repo_path: Path, org_name: str) -> None:
    ...
    safe_mkdir(base_path / "plugins" / "modules", repo_path, parents=True)
    safe_mkdir(base_path / "plugins" / "filter", repo_path, parents=True)
    safe_mkdir(base_path / "roles", repo_path, parents=True)
    safe_mkdir(base_path / "playbooks", repo_path, parents=True)
    safe_mkdir(base_path / "docs", repo_path, parents=True)
    safe_mkdir(base_path / "tests", repo_path, parents=True)
...
"""
    safe_write_text(base_path / "galaxy.yml", repo_path, galaxy_yml)
...
```
"""
    safe_write_text(base_path / "README.md", repo_path, collection_readme)


def _create_mono_repo_structure(repo_path: Path) -> None:
    """Create multi-project mono-repo structure."""
    repo_path = _ensure_within_base_path(repo_path, _get_workspace_root())
    safe_mkdir(repo_path / "inventories" / "prod", repo_path, parents=True)
    safe_mkdir(repo_path / "inventories" / "nonprod", repo_path, parents=True)
    safe_mkdir(repo_path / "projects" / "app1" / "playbooks", repo_path, parents=True)
    safe_mkdir(repo_path / "projects" / "app1" / "roles", repo_path, parents=True)
    safe_mkdir(repo_path / "shared_roles", repo_path)
    safe_mkdir(repo_path / "collections", repo_path)
...
"""
    safe_write_text(
        repo_path / "projects" / "app1" / "playbooks" / "deploy.yml",
        repo_path,
        app1_playbook,
    )
...
def generate_ansible_repository(
    ...
    if safe_exists(repo_path, _get_workspace_root()):
        return {
            "success": False,
    ...
    try:
        ...
        safe_mkdir(repo_path, _get_workspace_root(), parents=True)
        ...
        files_created = [
            ...
            for p in safe_glob(repo_path, "**/*", repo_path)
            if p.is_file()
        ]
...
def create_ansible_repository_from_roles(
    ...
    try:
        ...
        if not safe_exists(roles_dir, _get_workspace_root()):
            return {
                "success": False,
        ...
        role_dirs = [
            path
            for path in safe_iterdir(roles_dir, _get_workspace_root())
        ...
        repo_path = _validated_candidate(
            _normalize_path(str(repo_result["repo_path"])),
            _get_workspace_root(),
        )
