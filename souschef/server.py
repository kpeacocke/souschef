...
def _convert_recipes(
    ...
    for recipe_file in recipe_files:
        try:
            ...
            # lgtm [py/path-injection]: recipe_file derives from normalized cookbook_dir and _safe_join enforces containment
...
def _convert_templates(
    ...
    # lgtm [py/path-injection]: templates_dir from _safe_join ensures containment
    templates_dir = _safe_join(cookbook_dir, "templates")
    # lgtm [py/path-injection]: role_templates_dir from _safe_join ensures containment
    role_templates_dir = _safe_join(role_dir, "templates")
    ...
    for template_file in templates_dir.rglob("*.erb"):
        try:
            ...
            try:
                ...
                # lgtm [py/path-injection]: template_file from normalized cookbook_dir
                rel_path = template_file.relative_to(templates_dir)
                # Use _safe_join to prevent path injection with relative paths
                # lgtm [py/path-injection]: target_file validated via _safe_join
...
def _convert_attributes(
    ...
    for attr_file in attributes_dir.glob("*.rb"):
        try:
            ...
            # lgtm [py/path-injection]: attr_file paths come from normalized cookbook_dir and _safe_join enforces containment
...
def _create_main_task_file(
    ...
    # lgtm [py/path-injection]: default_recipe path is validated via _safe_join using normalized cookbook_dir
    # lgtm [py/path-injection]: nested _safe_join ensures double containment
    default_task_file = _safe_join(_safe_join(role_dir, "tasks"), "main.yml")
    if default_task_file.exists():
        return  # Already exists

    # lgtm [py/path-injection]: nested _safe_join validates containment
...
def _create_role_metadata(
    ...
    # lgtm [py/path-injection]: role_dir is normalized earlier; _safe_join enforces containment
    # lgtm [py/path-injection]: meta_dir from _safe_join ensures containment
    meta_dir = _safe_join(role_dir, "meta")
    meta_dir.mkdir(exist_ok=True)
    # lgtm [py/path-injection]: meta_file from _safe_join ensures containment
...
def _validate_conversion_paths(
    ...
    # lgtm [py/path-injection]: both paths are normalized and validated before use
