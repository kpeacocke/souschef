        ...
        if match:
            ...
            if module_name not in known_modules:
                self._add_result(
                    ...
                    suggestion="Verify module name and availability in target Ansible version",
                )
...
def _extract_attributes(content: str) -> list[dict[str, str]]:
    ...
    pattern = r"(default|force_default|normal|override|force_override|automatic)((?:\[[^\]]+\])+)\s*=\s*([^\n]+)"
...
# Assessment and Migration Planning Tools (imported from souschef.assessment)
