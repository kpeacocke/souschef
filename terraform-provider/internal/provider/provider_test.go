...
// Fixtures are located in tests/integration/fixtures/ after test reorganisation.
func getFixturePath(fixtureName string) string {
	...
	if !ok {
		...
		cwd, _ := os.Getwd()
		return filepath.Join(cwd, "..", "..", "tests", "integration", "fixtures", fixtureName)
	}

	// Navigate from provider_test.go location to repo root
	// provider_test.go is in: terraform-provider/internal/provider/
	repoRoot := filepath.Join(filepath.Dir(filename), "..", "..", "..")
	fixturesPath := filepath.Join(repoRoot, "tests", "integration", "fixtures", fixtureName)
